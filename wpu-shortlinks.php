...
add_action('publish_post', 'wpu_get_shortlink');
add_action('edit_post', 'wpu_get_shortlink');
add_action('save_post', 'wpu_get_shortlink');
...
function wpu_get_shortlink($post_id){
	global $post , $WPU_API_URL;
	...
	if(empty($wpu_shortlink)){
		...
		if ($result !== false) {			
			...
			if( is_wp_error( $response ) ) {
				...
				wpu_save_response($response['body'] , $post_id);
			}
		} else {
			$_SESSION['wpu_error'] = 'خطا در اتصال به سرویس!';
		}
