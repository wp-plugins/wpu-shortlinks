...
if(! function_exists("bloginfo")) return;
...
require_once(dirname(__FILE__).DIRECTORY_SEPARATOR."bulkaction.php");
...
?>
    ...
    <?php
    wp_enqueue_style('wpu_style', plugins_url("wpu-shortlinks/wpu-style.css",dirname(__FILE__)), array(), '', 'screen');
    wp_enqueue_script('wpu_script', plugins_url("wpu-shortlinks/wpu-js.js",dirname(__FILE__)), array('jquery'), '');
}
...
function wpu_content_urls($content){
    ...
    if($set['display_shortlink_content']){
        ...
        if(! empty($shortlink))
            $content .= '<br><br>'. __('Shortlink: ','wpu_shortlinks') . $shortlink;
    }
